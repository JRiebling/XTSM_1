...
class XTSM_core(object):
    ...
    def parse(self,additional_scope=None):
        ...
        try:        # try to directly convert to floating point
            ...
            try: 
                self._parseValue=eval(html.fromstring(self.PCDATA.replace('#','&')).text,__builtins__,tscope)  # evaluate expression
                suc=True
...
#        tsh[26:32]= Reserved for future use
    ...
    def construct_denseT(self):
        ...
        try: 
            ...
            # Remove final element if it exceeds the maximum time of the experiment. Needed for groups with clocking channels.
            if self.denseT[-1] > self.lasttimecoerced:
                self.denseT = numpy.delete(self.denseT, -1)
        except: pass
    ...
    def RepresentAsInteger(self):
        ...
        bits=bitarray([bool(ch.intedges[3,0]) for ch in self.channels.values()])
        ...
        if str.upper(str(self.tGroupNode.ParserInstructions.Pulser.PCDATA)) == 'YES': # for automatic pulses
            ...
            for ch in self.channels.values():
                if ch.intedges.shape[1] == 2:  # unused chanels are initialized with 2 times: beginning and end
                    ...
                    bits.append(1)
            bits = bitarray(bits)
            outvals[0] = numpy.fromstring(bits.tobytes(), dtype = dtype)
            outvals[-1] = numpy.fromstring(bits.tobytes(), dtype = dtype)
    ...
    def TimingStringConstruct(self):
        ...
        else:  # if the group outputs' are to be represented by single integer, the header must differ in calculation though not in content
            self.timingstring[8:15]=numpy.append(
    ...
    def generate_clockEdges(self,ctimes,chanObj,channelnumber):
        ...
        if not (str.upper(str(dTrain)) == 'YES'):
            # remove final fall time if it comes after the experiment ends
            if intedges[0][-1] > self.lasttimecoerced:
                intedges = numpy.delete(intedges, -1, 1)
        return intedges
...
class Interval(gnosis.xml.objectify._XO_,XTSM_core):
    ...
    def parse_harvest(self, dense_time_array):
        ...
        # now find changed values which are not on the interval's time resolution
        repeats = []
        for time in range(len(V)):
            test_exp = V[time] % time_res
        ...
        if hasattr(self,'_fasttag'): 
            ...
            # delete the incorrect updates in the array
            for elem in repeats:
                new_array = numpy.delete(new_array, elem, 1)
            return new_array
