...
function darken_all() {
	"use strict";
	/*
	Changes XTSM colors from dark to light or vice versa. VERY NONFUNCTIONAL.
	*/
	/*
	...
	*/
	...
	for (i = 0; i < divs.length; i++) {
		colors.push(divs[i].style.cssText);
		alert(divs[i].style.getPropertyValue(background));
	}
}
...
// Both editor and tree are inserted into the HTML DOM at the provided html_div.
    ...
    function refresh_tree() {
        ...
        var xslparser, docparser, xml, xsltProcessor, ex, exs, elem_num, elem, linkitem, divitem;
...
//                if (handler_name === "autocomplete") {alert(handler_args ); }
	...
	function jump_links() {
		...
		*/
		var tree, divs, i, children, parser2, xmlDoc2, value, name, search_result;
...
function main() {
	...
	*/
	...
	document.getElementById("light_dark_button").onclick = function () {does_nothing(); }; //DNWY (Obviously)
